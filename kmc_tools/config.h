...
class CAnalyzeOperationUsageDisplayer : public CUsageDisplayer
{
public:
	CAnalyzeOperationUsageDisplayer() : CUsageDisplayer("analyze"){}
	void Display() const override
	{
		std::cout << "kmer comparison, create a matrix of shared K-mers between two KMC's databases\n"
				  << "General syntax:\n"
				  << "kmc_tools analyze <input1:read K-mers> <intput2:assembly_K-mers>\n"
				  << "input1, input2                    - paths to databases generated by KMC\n"
				  << "output1                           - paths to output matrix\n"
				  << "Example:\n"
				  << "kmc_tools analyze reads_kmers ref_kmers output \n";
	}
};
...
public:
	CUsageDisplayerFactory(CConfig::Mode mode)
	{
		...
		case CConfig::Mode::TRANSFORM:
			...
			break;
		case CConfig::Mode::ANALYZE:
			desc = std::make_unique<CAnalyzeOperationUsageDisplayer>();
			break;
		default:
			desc = std::make_unique<CGeneralUsageDisplayer>();
			break;
