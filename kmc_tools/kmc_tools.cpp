...
template<unsigned SIZE> class CTools
{
	...
	bool analyze()
	{
		...
		uint32_t max_cov = 500;
		uint32_t max_cov_asm = 6;
		uint64_t *occ_cnts = new uint64_t[max_cov * max_cov_asm];
		memset(occ_cnts, 0, sizeof(uint64_t) * max_cov * max_cov_asm);	
		if (!input1.Finished() && !input2.Finished()) {
			uint32 get1 = 0;
			uint32 get2 = 0;

			CKmer<SIZE> kmer2 = input2.data.kmers_with_counters[get2].kmer;
			uint32 counter2 = input2.data.kmers_with_counters[get2].counter;
			CKmer<SIZE> kmer1 = input1.data.kmers_with_counters[get1].kmer;
			uint32 counter1 = input1.data.kmers_with_counters[get1].counter;

			uint32 left1 = input1->NRecLeft();
			uint32 left2 = input2.NRecLeft();
			
			while (true)
			{
				if (kmer1 == kmer2)
				{
					uint32_t idx = counter1 >= max_cov ?  (max_cov - 1)* 6 : (counter1 - 1) * 6;
					uint32_t idx_rmd = counter2 >=  max_cov_asm? max_cov_asm - 1: counter2 - 1; 
					...
					if (--left1)
					{
						...
						kmer1 = input1.data.kmers_with_counters[get1].kmer;
						counter1 = input1.data.kmers_with_counters[get1].counter;
					}
					else
					{
						input1.data.get_pos = get1;
						if (!input1.Finished())
						{
							get1 = 0;
							kmer1 = input1.data.kmers_with_counters[get1].kmer;
							counter1 = input1.data.kmers_with_counters[get1].counter;
							left1 = input1.NRecLeft();
						}
						else
							break;
					}
					...
					if (--left2)
					{
						kmer2 = input2.data.kmers_with_counters[get2].kmer;
						counter2 = input2.data.kmers_with_counters[get2].counter;
					}
					else
					{
						input2.data.get_pos = get2;
						if (!input2.Finished())
						{
							get2 = 0;
							kmer2 = input2.data.kmers_with_counters[get2].kmer;
							counter2 = input2.data.kmers_with_counters[get2].counter;
							left2 = input2.NRecLeft();
